...
| 延时处理 | 启用后会按设定的延迟时间重新获取原消息内容，再开始处理流程适，用于频繁修改消息的频道/群组 |
...
| 选择的媒体类型 | 选择要**屏蔽**的媒体类型，注意Telegram API 对媒体文件的分类是固定的，主要就是这几种，图片 (photo)，文档 (document)，视频 (video)，音频 (audio)，语音 (voice)其中所有不属于图片、视频、音频、语音的文件都会被归类为"文档"类型。比如病毒文件(.exe)、压缩包(.zip)、文本文件(.txt)等，在 Telegram 中都属于"文档"类型。 |
