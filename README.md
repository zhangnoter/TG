...
>注意：以上增删改查操作，只对第一个绑定的规则生效，示例里是TG 新闻。若想对TG 阅读进行操作，需要先使用`/switch`，选择TG READ，再进行操作增删改查。也可以使用`/add_all`，`/replace_all`等指令同时对两条规则生效
...
替换规则
/replace <正则表达式> <替换内容> - 添加替换规则到当前规则
/replace_all <正则表达式> <替换内容> - 添加替换规则到所有规则
